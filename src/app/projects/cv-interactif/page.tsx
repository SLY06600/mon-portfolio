...
export default function CvInteractif() {
  const [activeTab, setActiveTab] = useState<
    "description" | "skills" | "experience" | "projects" | "education" | "contact"
  >("description");
        ...
        {activeTab === "projects" && (
          <div className="space-y-4">
            {cvData.personalProjects.map((proj, idx) => (
              <div key={idx} className="border rounded-lg p-4 bg-white shadow-sm hover:shadow-md transition">
        ...
        {activeTab === "education" && (
          <ul className="list-disc list-inside space-y-2 text-gray-700">
            {cvData.education.map((edu, idx) => (
              <li key={idx}>
                <span className="font-medium">{edu.degree}</span> — {edu.school} ({edu.period})
              </li>
            ))}
          </ul>
        )}
